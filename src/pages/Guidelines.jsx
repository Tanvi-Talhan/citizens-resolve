    ...
    <div className="container max-w-screen-lg mx-auto p-4">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-10">
        {guidelines.map((guideline, index) => (
          <div
            key={index}
            className={`relative flex flex-col items-center rounded-md shadow-md p-6 transition-all duration-300 ${activeIndex === index ? 'bg-white' : 'bg-black'
              }`}
          ...
          >
            ...
            <h2 className={`text-2xl font-semibold mb-2 ${activeIndex === index ? 'text-black' : 'text-white'}`}>
              {guideline.title}
            </h2>
            <p className={`text-base mb-4 text-center ${activeIndex === index ? 'text-black' : 'text-white'}`}>
              {guideline.description}
            </p>
            <div className="mt-auto">
              <button
                onClick={() => handleReadMore(index)}
                className="inline-block px-6 py-2 mt-4 bg-green-600 text-white rounded hover:bg-blue-400 hover:text-black transition"
              >
            ...
            {activeIndex === index && (
              <div className="absolute inset-0 flex flex-col">
                ...
                <div className="w-full md:w-1/2 bg-white text-left p-4 rounded-md shadow-md">
